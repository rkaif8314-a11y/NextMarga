...
- Keep `OPENAI_API_KEY` and `GEMINI_API_KEY` in Vercel environment variables only; never commit them.
...
The deployed `/api/*` functions validate request methods and bound user-controlled text before sending it to an AI provider. AI responses must be treated as untrusted output and validated before being used as structured data.
