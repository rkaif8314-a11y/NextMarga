...
export interface Opportunity {
  id: string; title: string; organization?: string; category: 'competition' | 'scholarship' | 'exam' | 'internship' | 'fellowship'; isVerified: boolean; isGovt: boolean; deadline: string; deadlineDisplay: string; timeRemainingBadge: string; fee: string; mode: string; eligibility: string; description: string; whyConsider: string; aiMatchReason: string; requiredDocs: string[]; timeline: { phase: string; title: string; description: string; status: 'current' | 'upcoming' | 'final' }[]; officialUrl?: string; accentColor?: string; minimumClass?: string | null; maximumClass?: string | null; minimumAge?: number | null; maximumAge?: number | null; states?: string[]; boards?: string[]; interests?: string[]; matchScore?: number;
}
