...
interface OpportunityRow {
  id: string; title: string; organization: string; category: Opportunity['category']; is_verified: boolean; is_govt: boolean; deadline: string | null; fee: string | null; mode: string | null; eligibility: string | null; description: string | null; why_consider: string | null; required_docs: string[] | null; official_url: string | null; minimum_class: string | null; maximum_class: string | null; minimum_age: number | null; maximum_age: number | null; states: string[] | null; boards: string[] | null; interests: string[] | null; status?: 'active' | 'closed' | 'upcoming' | 'archived';
}
...
const opportunitySelect = 'id,title,organization,category,is_verified,is_govt,deadline,fee,mode,eligibility,description,why_consider,required_docs,official_url,minimum_class,maximum_class,minimum_age,maximum_age,states,boards,interests,status';
...
export async function getVerifiedOpportunities(limit = 250): Promise<MatchableOpportunity[]> {
  const { data, error } = await supabase.from('opportunities').select(opportunitySelect).eq('is_verified', true).in('status', ['active', 'upcoming']).order('deadline', { ascending: true, nullsFirst: false }).limit(limit);
  if (error) throw new Error(error.message);
  return ((data ?? []) as OpportunityRow[]).map(mapOpportunity);
}

export async function getOpportunityById(id: string): Promise<Opportunity | null> {
  const { data, error } = await supabase.from('opportunities').select(opportunitySelect).eq('id', id).eq('is_verified', true).in('status', ['active', 'upcoming']).maybeSingle();
  if (error) throw new Error(error.message);
