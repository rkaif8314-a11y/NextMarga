    ...
    <div className="max-w-2xl mx-auto px-4 pt-4 pb-32 space-y-6 animate-fadeIn">
      <div className="flex items-center justify-between border-b border-slate-200 pb-4">
        ...
        <div className="flex items-center gap-2">
          {!isEditing ? (
            <button onClick={startEditing} className="inline-flex items-center gap-2 text-xs uppercase tracking-[0.15em] font-medium bg-sky-700 text-white hover:bg-sky-800 px-4 py-2 rounded-lg transition-all shadow-sm"><Pencil className="w-3.5 h-3.5" />Edit</button>
          ) : (
      ...
      <div className="flex flex-col items-center justify-center space-y-3 py-3">
        <div className="relative">
          {formData.avatarUrl ? (
            <img
              ...
              className="w-24 h-24 rounded-full object-cover border-2 border-slate-300 shadow-lg"
            />
          ) : null}
          <div className={`w-24 h-24 rounded-full border-2 border-slate-300 bg-sky-50 text-sky-800 shadow-sm items-center justify-center text-2xl font-semibold ${formData.avatarUrl ? 'hidden' : 'flex'}`}>
            {(formData.fullName || 'N').trim().charAt(0).toUpperCase()}
          </div>
          {isEditing && <button type="button" disabled={uploadingPhoto} onClick={() => fileInputRef.current?.click()} className="absolute bottom-0 right-0 p-2 bg-sky-700 text-white rounded-full shadow-md hover:bg-sky-800 transition-transform active:scale-95 disabled:opacity-50" title="Upload photo"><Camera className="w-3.5 h-3.5" /></button>}
        ...
        {isEditing && <div className="flex items-center gap-2"><button disabled={uploadingPhoto} onClick={() => fileInputRef.current?.click()} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-sky-700 text-white rounded-full text-[10px] font-mono uppercase tracking-wider disabled:opacity-50">{uploadingPhoto ? <span>Uploading…</span> : <><Upload className="w-3 h-3" />Upload Photo</>}</button><button disabled={uploadingPhoto} onClick={handleImageUrl} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 text-slate-950 border border-slate-200 rounded-full text-[10px] font-mono uppercase tracking-wider disabled:opacity-50"><LinkIcon className="w-3 h-3" />Image URL</button></div>}
        <div className="text-center space-y-1"><div className="mb-2 inline-flex items-center rounded-full border border-sky-200 bg-sky-50 px-2.5 py-1 text-[10px] font-semibold text-sky-800">Personalization: {formData.interests.length ? 'Active' : 'Not set'}</div><h2 className="font-serif-luxury font-medium text-slate-950 text-lg">{formData.fullName}</h2><p className="text-xs font-mono text-slate-500">{formData.currentClass} Scholar</p><button onClick={onStartOnboarding} className="text-[10px] uppercase tracking-[0.15em] font-mono text-slate-700 hover:text-slate-950 mt-1 inline-flex items-center gap-1.5 border border-slate-200 px-3 py-1 rounded bg-slate-50"><Sparkles className="w-3 h-3 text-slate-700" />Re-run Guided Wizard</button></div>
      </div>
