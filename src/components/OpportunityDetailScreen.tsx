...
export const OpportunityDetailScreen: React.FC<OpportunityDetailScreenProps> = ({ opportunity, isSaved, onBack, onToggleSave, onApply }) => {
  ...
  return <div className="min-h-screen bg-[#0A0A0A] text-[#F5F2ED] pb-32 animate-fadeIn">
    ...
    <div className="fixed bottom-0 left-0 right-0 z-40 bg-[#0A0A0A]/95 backdrop-blur-xl border-t border-white/10 p-4"><div className="max-w-2xl mx-auto flex items-center gap-3"><button onClick={onToggleSave} className={`flex items-center justify-center gap-2 py-3.5 px-4 rounded-lg border text-xs uppercase tracking-[0.15em] ${isSaved ? 'bg-white text-black border-white' : 'bg-white/5 border-white/15 text-[#F5F2ED]'}`}><Bookmark className={`w-3.5 h-3.5 ${isSaved ? 'fill-black' : ''}`} /><span>{isSaved ? 'Saved' : 'Save'}</span></button>{opportunity.officialUrl ? <a href={opportunity.officialUrl} target="_blank" rel="noopener noreferrer" onClick={() => onApply?.()} className="flex-1 flex items-center justify-center gap-2 py-3.5 px-6 rounded-lg bg-[#F5F2ED] hover:bg-white text-black font-medium text-xs uppercase tracking-[0.15em] text-center"><span>Apply on Official Portal</span><ExternalLink className="w-3.5 h-3.5" /></a> : <button disabled className="flex-1 py-3.5 px-6 rounded-lg bg-white/10 text-white/35 text-xs uppercase tracking-[0.15em]">Official portal unavailable</button>}</div></div>
  </div>;
};
