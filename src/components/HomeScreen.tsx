...
export const HomeScreen: React.FC<HomeScreenProps> = ({ profile, opportunities, opportunitiesLoading = false, savedOpportunityIds, onSelectOpportunity, onToggleSave, onNavigate }) => {
  ...
  const filteredOpps = useMemo(() => {
    ...
    const result = opportunities.filter((opp) => {
      ...
      return (!q || haystack.includes(q)) &&
        ...
        (!verifiedOnly || opp.isVerified) &&
        (!governmentOnly || opp.isGovt) &&
        (!dueWithin7Days || isDueWithin7Days);
    });
  ...
  return <div className="max-w-2xl mx-auto px-4 pt-4 pb-24 space-y-6 animate-fadeIn">
    ...
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
      ...
      ].map((stat) => <button key={stat.label} onClick={() => selectStat(stat.key)} aria-pressed={activeStat === stat.key} className={`text-left p-3 rounded-xl border transition-all ${activeStat === stat.key ? 'bg-white text-black border-white shadow-[0_0_0_1px_rgba(255,255,255,0.25)]' : 'bg-[#121212] border-white/10 hover:border-white/25'}`}><div className={`text-[9px] uppercase tracking-[0.18em] ${activeStat === stat.key ? 'text-black/50' : 'text-white/40'}`}>{stat.label}</div><div className="mt-1 text-lg font-serif-luxury">{stat.value}</div><div className={`mt-0.5 text-[8px] uppercase tracking-wider ${activeStat === stat.key ? 'text-black/50' : 'text-white/30'}`}>{activeStat === stat.key ? 'Active filter • click to clear' : 'Click to filter'}</div></button>)}
    </div>
