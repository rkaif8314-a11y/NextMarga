...
import { ArrowLeft, Bell, Check, ChevronRight, LogOut, Moon, Palette, RotateCcw, Shield, Sparkles, Sun, UserRound } from 'lucide-react';
...
export const SettingsScreen: React.FC<SettingsScreenProps> = ({ onBack, onNavigate, onPreferencesChange, onSignOut }) => {
  ...
  const handleSignOut = () => {
    if (window.confirm('Sign out of your NextMarga account?')) onSignOut?.();
  };
  return <div className="min-h-screen bg-[#0A0A0A] text-[#F5F2ED] px-4 py-6 sm:px-6 pb-28">
    <div className="max-w-3xl mx-auto">
      ...
      <section className="mt-4 rounded-2xl border border-red-400/15 bg-red-400/[0.03] overflow-hidden">
        <div className="px-5 py-4 border-b border-red-400/10"><div className="text-xs uppercase tracking-[0.22em] font-mono text-red-200/60">Session</div><p className="text-[11px] text-white/35 mt-1">Sign out of this NextMarga account on this device.</p></div>
        <button onClick={handleSignOut} className="w-full flex items-center justify-between px-5 py-4 text-left hover:bg-red-400/5 transition-colors"><span className="flex items-center gap-3"><span className="flex items-center justify-center w-8 h-8 rounded-lg border border-red-300/15 bg-red-300/5"><LogOut className="w-4 h-4 text-red-200/70" /></span><span><span className="block text-xs text-red-100/90">Log out</span><span className="block text-[11px] text-white/35 mt-1">End your current session securely</span></span></span><ChevronRight className="w-4 h-4 text-white/30" /></button>
      </section>
