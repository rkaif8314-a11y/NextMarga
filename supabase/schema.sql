...
create table if not exists public.opportunities (
  ...
  category text not null check (category in ('competition','scholarship','exam','internship','fellowship','job','other')),
...
create table if not exists public.applications (
  ...
  status text not null default 'Saved' check (status in ('Under Review','Interview Scheduled','Applied','Saved','Accepted')),
...
create table if not exists public.notifications (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users(id) on delete cascade,
  type text not null check (type in ('urgent','match','update','milestone')),
  title text not null,
...
-- Schools and verified opportunities are public-read for the app.
...
create policy "Anyone can read verified opportunities"
on public.opportunities for select
using (is_verified = true);

-- Users can manage only their own applications, notifications and roadmap.
...
create policy "Users manage own roadmap goals"
on public.roadmap_goals for all
using (auth.uid() = (select user_id from public.roadmap_phases where id = phase_id))
with check (auth.uid() = (select user_id from public.roadmap_phases where id = phase_id));
